...
impl VaultCtx {
    ...
    /// `admin_cancel_withdrawal` as the admin on `request_owner`'s request.
    pub fn admin_cancel_withdrawal(
        ...
        let admin = self.admin.insecure_clone();
        self.admin_cancel_withdrawal_as(
            &admin,
            request_owner,
            request_id,
            expected_sequence,
    ...
    pub fn admin_cancel_withdrawal_as(
        &mut self,
        signer: &Keypair,
        request_owner: &Pubkey,
        request_id: u64,
        ...
        let accounts = self.admin_cancel_withdrawal_accounts(
            &signer.pubkey(),
            request_owner,
            request_id,
            destination,
        );
        self.send_admin_cancel_withdrawal(signer, accounts, expected_sequence)
    }

    pub fn admin_cancel_withdrawal_accounts(
        &self,
        admin: &Pubkey,
        request_owner: &Pubkey,
        request_id: u64,
        destination: Pubkey,
    ) -> q_accounts::AdminCancelWithdrawal {
        q_accounts::AdminCancelWithdrawal {
            queue: self.withdrawal_queue_pda(),
            vault_state: self.vault_state,
            admin: *admin,
    ...
    pub fn send_admin_cancel_withdrawal(
        &mut self,
        signer: &Keypair,
        accounts: q_accounts::AdminCancelWithdrawal,
        expected_sequence: u64,
    ) -> Result<litesvm::types::TransactionMetadata, FailedTransactionMetadata> {
        let ix = Instruction {
            program_id: august_withdrawal_queue::ID,
            accounts: accounts.to_account_metas(None),
            data: q_ix::AdminCancelWithdrawal { expected_sequence }.data(),
        };
        send_tx(&mut self.svm, signer, &[ix], &[signer])
    }
