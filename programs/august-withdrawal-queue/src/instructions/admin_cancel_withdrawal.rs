...
//! block `close_vault`. Only once the vault is released: while the queue is
//! live no one but the owner may touch a request, so this can never reset a
//! waiting user, and a released owner could redeem instantly anyway.
...
/// Admin, released vault, matching stamp; then the same settlement as the
